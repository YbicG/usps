def run():
    ...
    @tree.command(name="track", description="Track package with auto updates in a specific channel.")
    @discord.app_commands.describe(
        ...
        message ="The message to send along with the updates.",
        timeout="The time in seconds between messages. (Default 30)."
    )
    async def first_command(interaction, tracking_number_list: str, channel: str, message:str, timeout:int = 3600):

        trackLevel = 0

        isDoneWithLastMessage = True
        ...
        try:
            ...
            await interaction.response.send_message(content="**Invalid Channel**")
            return

        if timeout < 3600:
            await interaction.response.send_message(content="**Timeout must be greater than or equal to 3600 seconds (1h)!**")
            return
        ...
        while True:
            ...
            time.sleep(timeout)
            print(f"{style.GREEN}[TRACKING-{str(trackLevel)}]:{style.NONE} {style.MAGENTA}"+message+f"{style.NONE} {style.YELLOW}Last Updates Done: "+str(isDoneWithLastMessage)+style.NONE)
